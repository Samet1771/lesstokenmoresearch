...
# google cse. Two of those rate-limit hard by IP, and once they are suspended
...
def ensure_settings(force: bool = False) -> Path:
    """Write settings.yml if missing or outdated.

    The secret key is generated once and kept, so regenerating does not log
    anyone out of their own instance.
    """
    ...
    if path.exists() and not force:
        ...
        if version >= SETTINGS_VERSION:
            return path
        # Outdated: rewrite, but keep the key this instance already uses.
        for line in text.splitlines():
    ...
    lines = [
        ...
        f"{VERSION_MARKER} {SETTINGS_VERSION}",
